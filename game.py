...
async def validate_guess(guess, channel: discord.TextChannel, user_id: int):
    ...
    if guess == day.clong:
        ...
        if user_guesses[user_id][0] <= 2:
            await channel.send(f"""Incorrect! Try again. You are on guess {user_guesses[user_id][0]} out of 3. Cxei: {'🟩' if is_cxei and day.is_cxei else '⬛'}""")
...
async def remove_timed_out():
    ...
    for user_id in to_remove:
        del user_guesses[user_id]
        user_to_dm =  await client.fetch_user(user_id)
        await user_to_dm.send("Your guess has timed out.")
