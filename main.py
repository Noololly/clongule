...
class ClonguleModal(Modal, title='Clongule Test'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.user.send(f"You sent: {self.text_input}")
...
async def send_message(ctx):
    view = ModalButton()
    await ctx.send("Click the button to open the test Modal", view=view)

@client.command(name="select")
async def send_message(ctx):
    view = game.GuessView()
    await ctx.send("Test the selection box", view=view)
